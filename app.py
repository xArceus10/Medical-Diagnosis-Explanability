...
st.title("X-Ray Diagnosis with Explainability")
...
try:
    ...
    st.write("Pre-trained custom model loaded successfully.")
...
def compute_counterfactual(model, instance, target_shape=(224, 224, 3)):
    try:
        # Wrap the TensorFlow model for Alibi
        wrapped_model = tf.keras.Sequential([model])
...
uploaded_file = st.file_uploader("Upload a X-Ray Image", type=["jpg", "png"])
if uploaded_file:
    try:
